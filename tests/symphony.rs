...
const REL_ERR_THRESHOLD: f64 = 1e-4;
...
fn compare_powerlaw_subset(coeff: rimphony::Coefficient, cname: &str, index: usize, fraction_to_check: f64) {
    const NU: f64 = 1e9;
    const N_E: f64 = 1.;
...
fn compare_powerlaw_subset_ji() {
    compare_powerlaw_subset(Coefficient::Emission(Stokes::I), "J_I", 3, 0.0002);
}

#[test]
fn compare_powerlaw_subset_jq() {
    compare_powerlaw_subset(Coefficient::Emission(Stokes::Q), "J_Q", 5, 0.0002);
}

#[test]
fn compare_powerlaw_subset_jv() {
    compare_powerlaw_subset(Coefficient::Emission(Stokes::V), "J_V", 7, 0.0002);
}
